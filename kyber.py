...
class KyberPKE():
    ...
    Example usage:
        ...
        message = kyber_object.decrypt_message(encrypted_message, private)
    """

    def __init__(self, n: int = 256, q: int = 3329, k: int = 3, eta1: int = 2,
                 eta2: int = 2) -> None:
        """Create KyberPKE object to use generate_keys, encrypt_message,
        decrypt_message methods.
    ...
    def generate_keys(self) -> tuple[
            tuple[PolynomialMatrix, PolynomialMatrix], PolynomialMatrix]:
        """Generate public and private key.

        Returns public key: (A, s) and private key: s.
        """
        A = PolynomialMatrix(self.q, self.k, self.k, self.q, self.n)
        s = PolynomialMatrix(self.q, self.k, 1, self.eta1, self.n, True)
        e = PolynomialMatrix(self.q, self.k, 1, self.eta2, self.n, True)
        t = (A * s) + e
    ...
    def encrypt_message(self, message: str, public_key:
        ...
        r = PolynomialMatrix(self.q, self.k, 1, self.eta1, self.n, True)
        e1 = PolynomialMatrix(self.q, self.k, 1, self.eta2, self.n, True)
        e2 = PolynomialMatrix(self.q, 1, 1, self.eta2, self.n, True)

        # Scaling replaces all the "1" bits with q/2
        scaling_factor = round(self.q / 2)
        polynomial_message = \
            self._transform_message_to_polynomial(message, scaling_factor)
    ...
    def decrypt_message(self, encrypted_message:
        ...
        message_binary = \
            self._transform_polynomial_to_binary_string(message_polynomial)
        message = self._transform_binary_string_to_text(message_binary)

        return message

    def _transform_message_to_polynomial(self, message: str,
                                         scaling_factor: int) \
            -> "PolynomialMatrix":
        ...
        polynomial_message.matrix[0][0] = \
            [scaling_factor * int(c) for c in binary_message]

        return polynomial_message

    def _transform_message_to_binary(self, message: str) -> str:
        """Transform message to binary form and take only
    ...
    def _transform_polynomial_to_binary_string(self, poly_message:
                                               PolynomialMatrix) -> str:
        """Transform decrypted message polynomial to a string, binary
        representation.

        We need to center and round output to decide if it's closer to 0 or
        q/2. Centering: number 3300 is closer to q/2, but in mod 3309 it's
        closer to 0.
        """
    ...
    def _transform_binary_string_to_text(binary_string: str) -> str:
        ...
        return ''.join(chr(int(binary_string[i*8:i*8+8], 2))
